...
source("nearest_segment_model_sim.R")
...
for (error in c(0,5,10,20)){
  for (sim in 1:100){
    ...
    if (!file.exists(paste0("SimModels/nearest_segment_model_sim_",error,"_sim",sim,".rda"))){
      ...
      mcmc.output <- nimbleMCMC(nearest_segment_model_sim, data = data, inits = inits, constants = constants,
                                monitors = c("beta1",
                                             "beta2",
                                             "s",
                                             "lambda"), thin = 10,
      ...
      save(mcmc.output,file=paste0("SimModels/nearest_segment_model_sim_",error,"_sim",sim,".rda"))
